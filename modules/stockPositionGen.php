...
if(isset($_POST['id'])){
	...
	if( !($result = $connect->query($sql) ) ){
		var_dump ("Ошибка исполнения запроса в БД (stockPsitionGen 1)");
		die();
	}	
	...
	$produсtSize = $product['size'];
		...
		// реализуем запрос
		if(!($result = $connect->query($sql))){
			var_dump  ("Ошибка исполнения запроса в БД 2 (stockPsitionGen 2)");
			die();
		}		
	...
	// реализуем запрос
	if(	!($result = $connect->query($sql) ) ){
			var_dump ("Ошибка исполнения запроса в БД 2 (stockPsitionGen 3)");
			die();
	}
